...
# --- ANA PROGRAM ---
st.set_page_config(page_title="Klinik 2026 Yönetim", layout="wide")
...
if check_password():
    ...
    with col_main:
        ...
        search_term = st.text_input("🔍 Hızlı Arama (Hasta, Kategori...):", "")
        if search_term:
        ...
        for _, row in df_display.iterrows():
            ...
            if btn_e.button("✏️", key=f"e_{row.iloc[0]}"):
                ...
                def edit_modal(r_data):
                    ...
                    # Sıralama değiştirildi: Hasta adı en üstte, takvim otomatik açılmaz
                    ...
                    if st.button("Güncelle", use_container_width=True):
                        if n_tut <= 0:
                            st.error("Lütfen 0'dan büyük bir tutar giriniz!")
                        else:
            ...
            if btn_d.button("🗑️", key=f"d_{row.iloc[0]}"):
                ...
                def delete_modal(r_data):
                    st.warning(f"{r_data.iloc[3]} adına ait bu işlem silinecek. Emin misiniz?")
                    if st.button("Evet, Kaydı Sil", use_container_width=True):
                        idx = df_raw[df_raw.iloc[:,0] == r_data.iloc[0]].index[0] + 2
    ...
    with col_side:
        ...
        with st.form("form_v21", clear_on_submit=True):
            ...
            if st.form_submit_button("Sisteme Ekle", use_container_width=True):
                if f_tut <= 0:
